...
neo.viz = function(el, graph, layout, style) {
  ...
  viz.update = function() {
    ...
    height = (function() {
      try {
//          console.log(parseInt(el.style('height').replace('px', '')));
        return parseInt(el.style('height').replace('px', ''));
      } catch (_error) {}
    })();
    width = (function() {
      try {
//          console.log(parseInt(el.style('width').replace('px', '')));
...
//    var zoom = function(){
//        layers.enter().append("g").attr("transform", d3version4.event.transform);
//    };
    nodes = graph.nodes();
    relationships = graph.relationships();
    ...
    nodeGroups.enter().append("g").attr("class", "node").call(force.drag).call(clickHandler).call(d3.helper.tooltip()
                .attr({class: function(d, i) { return d + ' ' +  i + ' A'; }})
                .style({"background-color":'white',color: 'black',padding: '10px'})
                .text(function(d, i){ 
//                    console.log(d);
//                    console.log(i);
                    return 'Degree: '+d.weight; 
                })
            )
